...
public class ItemService {
    ...
    public void parseRecords(String[] records) {
        for (String record : records) {
            ...
            this.items.add(
                new Item(
                    DeL33t.translate(fields[0]),
                    (!fields[1].equals("")) ? Double.parseDouble(fields[1]) : null,
                    fields[2]
                )
            );
        }
...
//        }
    ...
    public String inventoryReport() {
        ...
        for (String name : items) {
            Item[] pricePoints = this.inventory.keySet().stream()
                    .filter(x-> x.getName().equals(name))
                    .collect(Collectors.toList())
                    .toArray(new Item[0]);
            int totalNum = Arrays.stream(pricePoints)
                    .mapToInt(x -> this.inventory.get(x))
                    .sum();
            String header = "";
        ...
        int totalNum = this.inventory.get(new Item());
